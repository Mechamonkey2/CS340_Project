...
/* Both Weapon_Stat and Armor_Stat are set up so that a piece of gear,
	let say a helmet, can have both the basic stats, but also another stat, like
	one that only increases magic as well. This means that it can be a M:M relationship
...
VALUES ((Select gearID from Item_IDs where gearID='1'),'Helmet'), ((Select gearID from Item_IDs where gearID='2'),'Helmet'), ((Select gearID from Item_IDs where gearID='3'),'Chest Piece'),
((Select gearID from Item_IDs where gearID='4'),'Chest Piece'), ((Select gearID from Item_IDs where gearID='5'),'Leggings'), ((Select gearID from Item_IDs where gearID='6'),'Leggings'),
...
((Select gearID from Item_IDs where gearID='10'),'Chest Piece'),((Select gearID from Item_IDs where gearID='11'),'Sword'),((Select gearID from Item_IDs where gearID='12'),'Bow'),
...
(3,300,300,'Gives 10% more physical defense on hit for 5 secounds. Does not stack',100,'Unique'),
(4,500,500,'Gives 1% of max hp per hit',100,'Legendary'),
(5,1,1,null,20,'Common'),
(6,100,100,'Gives 1 hp regen per second when worn',100,'Rare');
...
('3','Guard Chest Piece','Common','A common guard chest piece'),
('4','Adventurer Chest Piece','Common','A new adventurer chest piece'),
('5','Fallen Adventurer Chest Piece','Unique','A custom made leggings for a fallen adventurer'),
('6','Gaia Earth Legging','Unique','Famous brand of legging armor form the capital of Gaia'),
('7','Grand Old Boots','Legendary','The boots of a grand fallen hero. Has an interesting hole in the center'),
('8','Guard Boots','Common','A common guard boots'),
('9','Old Guard Boots','Legendary','An old set of boots from a long forgotten hero'),
('10','Starter Adventurer Chest Piece','Common','A common adventurer starting chest piece');
...
VALUES (1,'Health','Recovers the users health'),
(2,'Mana','Recovers the users Mana');

INSERT INTO Consumables(consumableID,name,recovery,recoveryTypeID,quantity,description)
VALUES ((Select consumableID from Item_IDs where consumableID='1'),'Small Health Potion',50,1,5,'A smaller version of the Health Potions'),
